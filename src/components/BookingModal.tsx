...
import { Pet, ServiceProvider, Booking } from '../types';
...
export const BookingModal: React.FC<BookingModalProps> = ({
  ...
  const [petId, setPetId] = useState(selectedPet?.id || (pets[0]?.id ?? ''));
  ...
  const currentPet = pets.find((p) => p.id === petId) || selectedPet || pets[0] || null;

  const handleConfirm = (e: React.FormEvent) => {
    e.preventDefault();
    const refCode = 'PW-' + Math.floor(100000 + Math.random() * 900000);
    setCreatedRef(refCode);

    const newBooking: Booking = {
      id: 'bk-' + Date.now(),
      petId: currentPet?.id || 'pet-custom',
      petName: currentPet?.name || 'My Pet',
      petPhoto: currentPet?.photoUrl || DEFAULT_PET_AVATAR,
      ...
      <div className="bg-[#fbf9f5] w-full max-w-[560px] rounded-3xl shadow-2xl border border-[#e5e0d8] overflow-hidden my-auto animate-in zoom-in-95 duration-200">
        ...
        {isSuccess ? (
          <div className="p-8 text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-sm text-[#544434] mt-1.5">
                Your booking reference is <strong className="text-[#895100]">{createdRef}</strong>.
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl p-5 border border-[#e5e0d8] text-left space-y-2 text-xs">
              <div className="flex justify-between pb-2 border-b border-[#f5f3ef]">
                ...
                <span className="font-bold text-[#1b1c1a]">{currentPet?.name || 'Pet'} ({currentPet?.breed || 'General'})</span>
              </div>
              <div className="flex justify-between pb-2 border-b border-[#f5f3ef]">
              ...
              <div className="flex justify-between pt-1">
                <span className="text-[#877462]">Amount:</span>
                <span className="font-bold text-[#895100]">₹{provider.priceNumber}</span>
              </div>
            </div>

            <button
              onClick={handleFinish}
              className="w-full py-3 rounded-full bg-[#ff9f1c] hover:bg-[#ff8f00] text-[#683c00] font-bold text-sm shadow-md transition-all cursor-pointer"
            >
          ...
          <form onSubmit={handleConfirm} className="p-6 space-y-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {pets.map((p) => (
                  ...
                  >
                    <img src={p.photoUrl || DEFAULT_PET_AVATAR} alt={p.name} className="w-8 h-8 rounded-full object-cover" />
                    <div>
            ...
            {/* Price Summary */}
            <div className="bg-[#efeeea] p-3 rounded-xl flex justify-between items-center text-xs">
              <span className="text-[#544434] font-medium">Estimated Total (incl. taxes)</span>
