    ...
    <header className="sticky top-0 z-40 bg-[#fbf9f5] border-b border-[#efeeea] shadow-xs">
      <div className="max-w-[1200px] mx-auto px-4 md:px-8 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          >
            <div className="w-9 h-9 rounded-full bg-[#895100] text-white flex items-center justify-center font-bold text-lg font-quicksand shadow-xs group-hover:scale-105 transition-transform">
          ...
          <nav className="hidden md:flex items-center gap-1 ml-4">
            <button
              ...
              onClick={() => setActiveTab('dashboard')}
              className={`px-3.5 py-1.5 rounded-full text-[15px] font-semibold transition-all cursor-pointer ${
                activeTab === 'dashboard'
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-[15px] font-semibold transition-all cursor-pointer flex items-center gap-1.5 ${
                activeTab === 'mypets'
                  ? 'text-[#895100] bg-[#ffdcbc]/40 font-bold'
                  : 'text-[#544434] hover:text-[#895100] hover:bg-[#efeeea]'
              }`}
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          {selectedPet ? (
            <div className="relative">
              ...
              >
                <img
                  src={selectedPet.photoUrl || DEFAULT_PET_AVATAR}
                ...
                <div className="hidden sm:flex flex-col">
                  ...
                  <span className="text-[10px] text-[#544434]">{selectedPet.breed?.split(' ')[0] || 'Pet'}</span>
                </div>
              </button>
                  ...
                  {pets.map((p) => (
                    <button
                      ...
                      className={`w-full px-3 py-2 flex items-center justify-between hover:bg-[#f5f3ef] transition-colors cursor-pointer text-left ${
                        selectedPet?.id === p.id ? 'bg-[#ffdcbc]/30' : ''
                      }`}
                    >
                      <div className="flex items-center gap-2.5">
                        <img src={p.photoUrl || DEFAULT_PET_AVATAR} alt={p.name} className="w-8 h-8 rounded-full object-cover border border-[#dac2ae]" />
                        <div>
            ...
            <button
              ...
              className="py-1 px-3 rounded-full bg-[#ffdcbc]/50 hover:bg-[#ffdcbc] text-[#895100] text-xs font-bold flex items-center gap-1 transition-colors cursor-pointer"
            >
          ...
          <div className="relative pl-1">
            {currentUser ? (
              ...
              <button
                ...
                className="py-1.5 px-3 rounded-full bg-[#f59e0b] hover:bg-[#ea580c] text-white text-xs font-bold flex items-center gap-1 shadow-2xs transition-colors cursor-pointer"
              >
            ...
            {showUserMenu && currentUser && (
              <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-[#efeeea] py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                <div className="px-4 py-3 border-b border-[#efeeea]">
                  <div className="text-sm font-bold text-[#1b1c1a] truncate">{currentUser.name}</div>
                  ...
                  <div className="mt-1 flex items-center gap-1 text-[11px] text-[#41674b] font-semibold">
                    ...
                    <span>Signed In ({currentUser.location})</span>
                  </div>
                </div>
                ...
                <div className="py-1">
                  ...
                  >
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#ffdcbc] text-[#895100] font-bold">New</span>
                  </button>
