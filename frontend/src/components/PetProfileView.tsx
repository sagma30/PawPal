...
export const PetProfileView: React.FC<PetProfileViewProps> = ({
  ...
  if (!pet) {
    return (
      <div className="w-full max-w-[800px] mx-auto px-4 md:px-8 py-16 text-center">
        <div className="w-20 h-20 mx-auto rounded-full bg-[#ffdcbc]/40 text-[#895100] flex items-center justify-center mb-4">
          <span className="material-symbols-outlined text-4xl">pets</span>
        </div>
        <h2 className="text-2xl font-bold font-quicksand text-[#1b1c1a] mb-2">No Pet Profile Selected</h2>
        <p className="text-sm text-[#544434] mb-6">Select an existing pet profile or add a new one to manage health records and live tracking.</p>
        <button
          onClick={() => onOpenEditProfile({} as any)}
          className="px-6 py-2.5 rounded-full bg-[#ff9f1c] hover:bg-[#ff8f00] text-[#683c00] font-bold text-sm shadow-md transition-all cursor-pointer"
        >
          Add Pet Profile
        </button>
      </div>
  ...
  const filteredEvents = (pet.healthEvents || []).filter((event) => {
  ...
  const handleRefreshLocation = () => {
    ...
    setTimeout(() => {
      ...
      setLocationData({
        ...
        status: randomStatus,
        lastUpdated: 'Just now',
        battery: Math.max(70, Math.min(100, (locationData?.battery || 85) - 1))
      });
    ...
    <div className="w-full max-w-[1200px] mx-auto px-4 md:px-8 py-6 md:py-10 space-y-10 animate-fade-in">
      ...
      {allPets.length > 1 && (
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
          ...
          {allPets.map((p) => (
            ...
            >
              <img src={p.photoUrl || DEFAULT_PET_AVATAR} alt={p.name} className="w-5 h-5 rounded-full object-cover" />
              <span>{p.name}</span>
            </button>
          ))}
        </div>
      ...
      <section className="flex flex-col md:flex-row gap-8 items-center md:items-start">
        ...
        <div className="w-36 h-36 md:w-44 md:h-44 rounded-full border-4 border-[#895100] overflow-hidden shadow-lg shrink-0 relative bg-white">
          <img
            src={pet.photoUrl || DEFAULT_PET_AVATAR}
            alt={pet.name}
          ...
          <div className="bg-white rounded-2xl p-6 md:p-8 border border-[#e5e0d8] shadow-xs">
            ...
            {filteredEvents.length === 0 ? (
              <div className="text-center py-12 text-[#877462] space-y-3">
                ...
                <button
                  onClick={() => onOpenAddHealthEvent(pet)}
                  className="text-xs font-bold text-[#895100] hover:underline"
                >
